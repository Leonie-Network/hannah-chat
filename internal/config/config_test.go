...
func TestLoadExampleConfig(t *testing.T) {
	...
	if cfg.Hannah.Address != "192.168.1.1:50051" {
		t.Errorf("hannah.address mismatch: %q", cfg.Hannah.Address)
	}
}

func TestLoadMissingFile(t *testing.T) {
	t.Setenv("HANNAH_CHAT_HANNAH__ADDRESS", "192.168.1.5:50051")
...
func TestOtherHannahPrefixedVarsAreNotAbsorbed(t *testing.T) {
	...
	t.Setenv("HANNAH_CHAT_HANNAH__ADDRESS", "192.168.1.6:50051")

	cfg, err := Load(filepath.Join(t.TempDir(), "does-not-exist.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Hannah.Address != "192.168.1.6:50051" {
		t.Errorf("hannah.address mismatch: %q", cfg.Hannah.Address)
	}
}
